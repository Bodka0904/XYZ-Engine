...
void TestLayer::OnAttach()
{
	...
	XYZ::Renderer::Init();
	//m_Camera = std::make_shared<XYZ::OrthoCamera>(0, XYZ::Input::GetWindowSize().first, 0, XYZ::Input::GetWindowSize().second);	
	...
	m_ParQuad = new ParticleQuad(glm::vec2(0), glm::vec2(0.2f));
	...
	for (size_t i = 0; i < particles.size(); ++i)
	{
		...
		particles2[i].sizeBegin = 1.0f;
		particles2[i].sizeEnd = 0.0f;
		particles2[i].colorEnd = glm::vec4(1, 0, 0, 0);
		particles2[i].colorBegin = glm::vec4(0, 0, 1, 1);
		particles2[i].velocity = glm::vec2(dist(rng), fabs(dist(rng)) * 3);
		particles2[i].lifeTime = fabs(dist(rng)) * 4;
	...
	XYZ::ECSManager::Get()->AddComponent(m_SpriteEntity, new XYZ::Sprite(glm::vec3(0,0,0),glm::vec2(1,1),glm::vec4(1,1,1,1),glm::vec4(0.0f,0.0f,1.0f/8.0f,1.0f/3.0f)));
